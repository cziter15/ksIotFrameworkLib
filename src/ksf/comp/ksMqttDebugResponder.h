...
namespace ksf
{
	...
	class ksMqttDebugResponder : public ksComponent, public std::enable_shared_from_this<ksMqttDebugResponder>
	{
		protected:
			...
			std::shared_ptr<ksEventHandle> connEventHandle;		//< Event handle for connection delegate.
			std::shared_ptr<ksEventHandle> msgEventHandle;		//< Event handle for message delegate.
		...
		public:
			DECLARE_KS_EVENT(customDebugHandler, ksf::ksMqttDebugResponder*, const String&, bool&) //< Event that provides custom message handler.
